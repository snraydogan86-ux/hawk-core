...
Kurallar (spec Bölüm 7):
  ...
  - Adapter ağırlığı GitHub'a KONMAZ → yalnız R2 key + SHA256 referansı.
...
R2 = "models/hawk-base-lora-{v}/adapter.tgz"
_BASE = "Qwen/Qwen3-8B"
_COMMON = dict(family="base", role="hawk_base", base_model=_BASE, adapter_type="qlora",
               ...
               supported_languages=("tr", "en"), license="apache-2.0 (Qwen3-8B) + CC0 (adapter/dataset)",
...
# GERÇEK SHA256 (adapter.tgz, 2026-07-14 hesaplandı; v0.4 yerel==R2 doğrulandı)
_SHA = {
...
def _mv(version, status, dataset_ver, final_loss, *, shadow=None, canary=None, note="",
    ...
    return ModelVersion(
        ...
        checkpoint=R2.format(v=version), adapter_sha256=_SHA.get(version, ""),
...
def training_proposal_v05() -> dict:
    ...
    return {
        ...
        "gpu_plan": {"provider": "RunPod on-demand", "est_usd_hr": 0.4, "launch_cap_usd": 3,
                     "daily_cap_usd": 8, "est_duration": "~10-15 dk (warm) veya ~20 dk (cold)",
                     "est_total_usd": "~0.4-0.8"},
        "post_train": ["v0.8 adapter → R2", "60-örnek benchmark re-run (tool_calling ≥0.70 hedef)",
...
def write_manifest(path: str = "") -> str:
    ...
    doc = {
        "generated_note": "HAWK Base sürüm zinciri (adapter ağırlığı R2'de, GitHub'a konmaz).",
