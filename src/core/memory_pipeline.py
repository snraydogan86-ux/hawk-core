...
Hiçbir endpoint doğrudan store çağırmaz; hepsi buradan geçer:
  write_message_facts(user_id, message, conversation_id, extra_facts)
     → extract_facts (saf)
     → write_fact  → 1) SQLite (kaynak, kritik)  2) PostgreSQL  3) cloud database
...
Kurallar:
 ...
 - Postgres/cloud database eventual-consistency: başarısızsa write_retry kuyruğuna girer.
 - IDEMPOTENT: SQLite/PG ON CONFLICT DO UPDATE; cloud database (user_ref,category) tekil.
...
async def write_fact(user_id: str, key: str, value: str, *, confidence: float = 0.9,
    ...
    res: Dict[str, Any] = {"sqlite": None, "postgres": None, "cloud": None}
    ...
    if not wrote:
        ...
        res["postgres"] = "skip"; res["cloud"] = "skip"
        return res
    ...
    # 3) cloud database (idempotent upsert)
    try:
        from core import cloud_store as _SB
        if _SB.is_ready():
            ...
            if not ok:
                _MS.queue_write_retry(user_id, key, value, confidence, source,
                                      conversation_id, extraction_method, "cloud")
        else:
            res["cloud"] = "inactive"
    except Exception:
        res["cloud"] = False
...
async def write_facts(user_id: str, facts: List[Dict], conversation_id: str = "") -> Dict[str, Any]:
    agg = {"count": 0, "sqlite": 0, "postgres": 0, "cloud": 0, "queued": 0, "per_fact": []}
    for f in facts or []:
        ...
        if r.get("postgres") is True:
            agg["postgres"] += 1
        if r.get("cloud") is True:
            agg["cloud"] += 1
        if r.get("postgres") is False or r.get("cloud") is False:
            agg["queued"] += 1
...
async def retry_pending_writes(max_items: int = 50) -> int:
    """write_retry kuyruğundaki başarısız pg/cloud yazımlarını idempotent tekrar dener."""
    done = 0
    try:
